...
```text
Agent          // public façade，创建 context/turn，提供高层 API；不是执行核心
AgentContext   // 可序列化长期上下文，拥有 history/config，不包含 model/tools
...
```text
AgentActor      // 技术实现名，删除
旧 AgentState   // 旧 public/monolithic state 概念，删除；当前 AgentState 仅是内部实现容器
...
src/context.rs        // public AgentContext + internal AgentState command loop
...
src/turn_machine.rs    // AgentTurnMachine pure state machine
```
...
- 创建和访问 `AgentContext`。
- 启动 `AgentTurn` 并返回 `AgentTurnHandle`。
- 暴露可调度 API，例如 `start_turn()`。
- 导出/恢复长期上下文值对象 `AgentContext`。
...
impl Agent {
    ...
    pub fn from_context(context: AgentContext, model: Arc<dyn Model>, tools: Arc<dyn ToolRouter>) -> Self;
...
`AgentContext` 是 agent 的可序列化长期上下文，不是执行器，也不是 live handle。

职责：

- 拥有 conversation history：`Vec<Message>`。
- 拥有可保存配置：`options`、`tool_choice`、`hosted_tools`。
- 字段保持私有，对外只提供 getter，避免被误解为 live mutable context。
- 可用于保存、恢复、fork agent session。
...
内部 `AgentState` 持有 `AgentContext + model + tools` 并运行 command loop。并发边界：第一版同一个 `AgentState` 只允许一个 active mutable `AgentTurn`。多 agent 调度通过多个 agent/context 组合实现。
...
- 保证外部调度器只能在 step boundary 控制 turn，不能直接改 `AgentTurnMachine` 或 context history。
- 可以被不同上层调度策略复用。
...
- 不允许多个 active mutable turn 隐式共享同一个 `AgentContext` 写 history。
...
- 调度器不能直接修改 `AgentTurnMachine`、`AgentTurnState`、in-flight handle 或 `AgentContext` history。
...
history 写入只能通过 `AgentTurn` 对 `AgentContext` 的受控 commit 完成。
...
- active turn 中并发执行 `replace_messages`。

第一版 mutation 规则：

- `push_message` / `replace_messages` / options/tool config mutation 只在没有 active mutable turn 时允许。
- 如果 turn `Paused` 且需要修改 context，必须通过 `AgentControl` 或显式 abort 当前 turn 后再修改。
...
- `Agent::start_turn()` 创建 `AgentTurnHandle`。
...
- 新增 `AgentContext`、`AgentTurn`、`AgentTurnMachine`、`AgentStepId`、`AgentAction`、`AgentOutcome`、`AgentControl`。
- 使用新的核心 `AgentEvent` 替换旧式 streaming 事件。
- `AgentTurnHandle::events()` 直接输出核心事件，作为破坏性迁移的一部分。

### Phase 2: 拆分长期状态和执行状态

- 把 `AgentState` 拆为 `AgentContext` 和 `AgentTurn` 内部状态。
...
- 不在第一版实现跨进程持久化恢复。
- 不在第一版允许同一 `AgentContext` 多个 active mutable turn 并发写 history。
