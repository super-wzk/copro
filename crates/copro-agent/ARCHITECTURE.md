...
内部 `AgentState` 持有 `AgentContext + model + tools + run ids` 并运行 command loop。并发边界：第一版同一个 `AgentState` 只允许一个 active mutable `AgentRun`。多 agent 调度通过多个 agent/context 组合实现。
...
- `FinishRun` 结束当前 run，`AbortRun` 直接产生 run aborted。
...
- `FinishRun` 结束当前 run，必要时进入 recovery。
...
```text
LoadTools/ToolsLoaded             -> Continue / Pause / FinishRun / AbortRun
BuildRequest/RequestBuilt         -> Continue / Pause / FinishRun / AbortRun / ReplaceRequest
ReadModelStream/ModelDelta        -> Continue / Pause / FinishRun / AbortRun / ReplaceModelDelta / DropModelDelta
ReadModelStream/ModelOutputFinished -> Continue / Pause / FinishRun / AbortRun / ReplaceAssistantOutput
PlanTool/ToolPlanned              -> Continue / Pause / FinishRun / AbortRun / ReplaceToolCall / RejectToolCall
PlanTool/ToolRejected             -> Continue / Pause / FinishRun / AbortRun / ReplaceToolCall / RejectToolCall
ReadTool/ToolFinished             -> Continue / Pause / FinishRun / AbortRun
...
FinishRun/RunFinished             -> Continue
```
