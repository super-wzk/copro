...
async fn list_path(
    ...
    match metadata.file_type {
        VfsFileType::File => output.push(display_path(root, &start)),
        VfsFileType::Directory => {
            if let Some(matcher) = load_gitignore_in_dir(&start).await? {
                gitignores.push(matcher);
            }
            list_directory(root, &start, &gitignores, output, cancel).await?;
        }
    }
...
async fn list_directory(
    ...
    entries.sort_by_key(|(path, metadata)| sort_key(root, path, metadata.file_type));
    ...
    for (entry_path, metadata) in entries {
        ...
        output.push(render_entry(root, &entry_path, metadata.file_type));
    }

    Ok(())
}

fn sort_key(root: &AsyncVfsPath, path: &AsyncVfsPath, file_type: VfsFileType) -> (u8, String) {
    let kind = match file_type {
        VfsFileType::Directory => 0,
        VfsFileType::File => 1,
    };
    (kind, display_path(root, path))
}

fn render_entry(root: &AsyncVfsPath, path: &AsyncVfsPath, file_type: VfsFileType) -> String {
    let mut entry = display_path(root, path);
