...
pub(crate) fn build_response_body(
    ...
    insert_request_options(&mut body, &request_options);
...
fn insert_request_options(body: &mut Map<String, Value>, options: &OpenAiResponsesRequestOptions) {
    insert_optional_json(
        body,
        "prompt_cache_key",
...
fn input_content_part(content: InputContent) -> Value {
    match content {
        ...
        InputContent::Image(ImageContent::Data { mime_type, data }) => {
            let image = base64::engine::general_purpose::STANDARD.encode(data.as_ref());
...
mod tests {
    ...
    #[test]
    fn maps_request_prompt_cache_options() {
        let mut options = empty_options();
        options
            .insert_extra(OpenAiResponsesRequestOptions {
                prompt_cache_key: Some("coox:v1:tools=workspace-v2".to_string()),
        ...
        let body =
            build_response_body("gpt-5", &OpenAiResponsesModelConfig::default(), request).unwrap();

        assert_eq!(body["prompt_cache_key"], "coox:v1:tools=workspace-v2");
